...
export class RouteGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // get admin
    const foundAdmin = await this.adminService.findAdmin(
      extractData.data.email,
    );
    if (!foundAdmin.data) {
      return false;
    }
    // get permision and role of this route from database
    const dbGetRouteRes = await this.routeService.findByRouteName(
      requestPath,
      requestMethod,
    );
    ...
    return (
      comparePermission(adminPermissions, routePermissions) &&
      compareRole(adminRoles, routeRoles)
    );
  }
}
