  ...
  async create(createOrderDto: CreateOrderDto): Promise<Result> {
    ...
    try {
      ...
      for (const cartItem of cartItems.data) {
        const childProduct = await this.childProductService.findOne(
        ...
        if (quantityInWarehouse < cartItem.data.quantity) {
          itemQuantityStatus.push({
            cartItem,
            availableQuantity: quantityInWarehouse,
          });
        }
      }
      ...
      for (const cartItem of cartItems.data) {
        const updateQttRes = await this.childProductService.updateQuantity(
          cartItem.data.child_product.id,
          cartItem.data.child_product.id - cartItem.data.quantity,
        );
      ...
      const updateCartRes = await this.cartService.updateStatus({
        cartId,
        status: "PROCESSING",
      });
      ...
      return new Result(
        Status.ERROR,
        null,
        error?.message || "Create order ERROR, rollback",
      );
    } finally {
